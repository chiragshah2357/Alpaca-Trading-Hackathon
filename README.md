...
- **Primary brain:** **Gemini** (Google AI Studio free tier — strong reasoning, huge context for RAG, no billing).
- **Fast fallback:** **Groq** (Llama 3.3 70B — ultra-fast, for cheap high-frequency steps like headline sentiment).
- **Swappable** via LangChain, so Claude/others can drop in later if credits ever appear.
...
- **State between runs** — each run is a fresh empty machine, so:
  ...
  - Thesis log → **commit back to the repo** each run (small JSON/SQLite). Persists *and* becomes a visible audit trail for judges.

Caveats (all minor): scheduled runs can be delayed a few min under load (irrelevant at 15–30 min cadence); min interval 5 min; workflow must be on the **default branch** to fire.
